 ...
 * @author wcc
 * @author 2016年6月12日
 * @author 增加ismobile 方法判断是否移动端。增加 table 上和 th 上两个 data-width 参数指定移动端时的表格宽度和列宽
 */
(function($){
	$.fn.jTable = function(options){
		return this.each(function(){
		 	...
		 	//判断是移动端才使用这个参数
		 	if(ismobile()){
		 		phonewidth = Number($table.attr('data-width')) || false;
		 	}else{
		 		phonewidth = false;
		 	}
		 	var tlength = phonewidth || $table.width();
			var aStyles = [];
			...
			for(var i = 0, l = oldThs.size(); i < l; i++) {
				...
				//如果是手机上。并且th上有 width 数据。使用指定的width
				if(ismobile()){
					if(Number($th.attr('data-width')) > 0){
						width = Number($th.attr('data-width'));
					}
				}
